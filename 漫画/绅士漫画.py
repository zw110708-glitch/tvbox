...
class Spider(Spider):
    ...
    def init(self, extend=""):
        ...
        if not isinstance(config, dict):
            config = {}
        self.baseUrl = config.get("site", "https://www.wn03.ru")
    ...
    def check_domain(self):
        try:
            if self.session.get(self.baseUrl, headers=self.headers, timeout=5, proxies=self.proxy).status_code >= 400:
    ...
    def fetch_new_domain(self):
        try:
            r = self.session.get("https://wnlink.ru/", headers=self.headers, timeout=5, proxies=self.proxy)
            urls = re.findall(r'href="(https?://[^"]+)"', r.text)
            for url in urls:
                if "wn" in url and "link" not in url:
                    self.baseUrl = url.rstrip('/')
                    self.session.get(self.baseUrl, headers=self.headers, timeout=5, proxies=self.proxy)
                    break
        except: pass
    ...
    def parse_list(self, url, pg):
        try:
            r = self.session.get(url, headers=self.get_header(url), timeout=10, proxies=self.proxy)
            ...
            for item in items:
                ...
                videos.append({"vod_id": a['href'], "vod_name": title, "vod_pic": (self.plp + pic) if pic else "", "vod_remarks": remark})
    ...
    def detailContent(self, ids):
        ...
        try:
            # 请求详情页以获取准确标题
            r = self.session.get(url, headers=self.get_header(self.baseUrl), timeout=10, proxies=self.proxy)
            r.encoding = 'utf-8'
            soup = BeautifulSoup(r.text, 'html.parser')
            ...
            return {"list": [{
                "vod_id": vid, "vod_name": title, "vod_pic": (self.plp + cover) if cover else "", "vod_type": "漫画",
                "vod_play_from": "绅士漫画$$$绅士漫画(Manga)",
                "vod_play_url": f"全集${self.plp + play_url}$$$全集${self.plp + play_url}"
            }]}
        except:
            return {"list": [{"vod_id": vid, "vod_name": "加载失败", "vod_play_from": "绅士漫画", "vod_play_url": f"全集${self.plp + url}"}]}
    ...
    def playerContent(self, flag, id, vipFlags):
        try:
            ...
            r = self.session.get(real_url, headers=self.get_header(real_url), timeout=15, proxies=self.proxy)
            ...
            if not img_list: return {"parse": 0, "url": "", "header": ""}
            
            # 每张图加 self.plp 前缀，返回 manga:// 协议
            proxied = [self.plp + u for u in img_list]
            return {
                "parse": 0, "playUrl": "",
