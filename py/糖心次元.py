...
class Spider(Spider):
    ...
    def _parse(self, rsp):
        ...
        for v in videos:
            ...
            if link and not link.startswith('http'):
                link = 'https://down.nigx.cn/www.txsp.my' + link
            lst.append({'vod_name': name or '未知标题', 'vod_pic': f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{img}', 'vod_remarks': (v.xpath('.//span[contains(@class,"ico-left")]/text()') or [''])[0].strip(), 'vod_id': link})
        return lst

    def categoryContent(self, tid, pg, filter, extend):
        url = f'https://down.nigx.cn/www.txsp.my/index.php/vod/type/id/{tid}.html' if pg == '1' else f'https://down.nigx.cn/www.txsp.my/index.php/vod/type/id/{tid}/page/{pg}.html'
        try:
    ...
    def detailContent(self, array):
        tid = array[0]
        url = tid if tid.startswith('http') else 'https://down.nigx.cn/www.txsp.my' + tid
        try:
            rsp = self.fetch(url).text
            root = etree.HTML(rsp)
            ...
            if pic and not pic.startswith('http'):
                pic = ('https:' + pic) if pic.startswith('//') else 'https://down.nigx.cn/img1.souavzy.org' + pic
            play_url = self._extract(rsp)
            return {'list': [{'vod_id': f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{tid}', 'vod_name': title, 'vod_pic': f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{pic}', 'vod_content': title, 'vod_play_from': '糖心次元', 'vod_play_url': '播放$' + play_url if play_url else '播放$暂无播放地址'}]}
        except Exception as e:
            return {'list': []}
    ...
    def searchContent(self, key, quick, pg="1"):
        url = f'https://down.nigx.cn/www.txsp.my/index.php/vod/search/page/{pg}/wd/{urllib.parse.quote(key)}.html'
        try:
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if id.startswith('http') and ('.m3u8' in id or 'souavzy' in id):
            return {"parse": 0, "playUrl": '', "url": f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{id}', "header": {"User-Agent": "Mozilla/5.0", "Referer": "https://www.txsp.my/", "Origin": "https://www.txsp.my"}}
        try:
            url = id if id.startswith('http') else 'https://down.nigx.cn/www.txsp.my' + id
            play_url = self._extract(self.fetch(url).text)
            if play_url:
                return {"parse": 0, "playUrl": '', "url": f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{play_url}', "header": {"User-Agent": "Mozilla/5.0", "Referer": "https://www.txsp.my/", "Origin": "https://www.txsp.my"}}
        except:
            pass
        return {"parse": 1, "playUrl": '', "url": f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{id}', "header": {"User-Agent": "Mozilla/5.0", "Referer": "https://www.txsp.my/", "Origin": "https://www.txsp.my"}}
