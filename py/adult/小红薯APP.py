...
class Spider(Spider):
    ...
    def proxy_url(self, url):
        ...
        return f"{proxy_prefix}{urllib.parse.quote(url)}"
    ...
    def homeContent(self, filter):
        # 使用代理URL
        data = self.fetch(
            self.proxy_url(f'{self.host}/api/video/queryClassifyList?mark=4'), 
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        # 使用代理URL
        data = self.fetch(
            self.proxy_url(f'{self.host}{path}'), 
            headers=self.headers()
        ).json()['encData']
        vdata = self.aes(data)
        for k in vdata['data']:
            videos.append({
                ...
                'vod_pic': self.getProxyUrl() + '&url=' + k['coverImg'],
    ...
    def detailContent(self, ids):
        ...
        vod = {
            'vod_name': v["title"], 
            'type_name': ''.join(v.get('tagTitles', [])),
            'vod_play_from': v.get('nickName') or "小红书官方", 
            'vod_play_url': d
        }
        result = {"list": [vod]}
        return result
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # 原始播放地址
        original_url = f"{self.host}/api/m3u8/decode/authPath?{id}"
        # 使用代理URL
        proxy_url = self.proxy_url(original_url)
        
        result = {"parse": 0, "url": proxy_url, "header": h}
        return result
