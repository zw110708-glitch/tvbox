...
class Spider(Spider):
    ...
    def fetch_with_retry(self, url, params=None, retry=1, timeout=5):
        for i in range(retry + 1):
            try:
                resp = self.fetch(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{urllib.parse.quote(url)}', headers=self.headers, timeout=timeout, params=params or {})
                if resp.status_code in (200, 301, 302):
    ...
    def fetch(self, url, headers=None, timeout=5, method='GET', params=None):
        ...
        try:
            if method.upper() == 'GET':
                return requests.get(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{urllib.parse.quote(url)}',
 headers=headers, timeout=timeout, allow_redirects=True, params=params)
            elif method.upper() == 'HEAD':
                return requests.head(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{urllib.parse.quote(url)}',
 headers=headers, timeout=timeout, allow_redirects=False, params=params)
            else:
                return requests.get(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{urllib.parse.quote(url)}', headers=headers, timeout=timeout, allow_redirects=True, params=params)
        except Exception as e:
