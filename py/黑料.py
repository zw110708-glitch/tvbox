...
class Spider(Spider):
    ...
    def homeVideoContent(self):
        videos = []
        try:
            res = requests.get(xurl + '/show/30/', headers=headerx,proxies=self.proxy)
            res.encoding = "utf-8"
            res = res.text
    ...
    def homeContent(self, filter):
        res = requests.get(xurl + '/zzzz', headers=headerx,proxies=self.proxy)
        res.encoding = "utf-8"
        res = res.text
        doc = BeautifulSoup(res, "html.parser")
        result = {}
    ...
    def categoryContent(self, cid, pg, filter, ext):
        ...
        res = requests.get(url=url, headers=headerx,proxies=self.proxy)
        res.encoding = "utf-8"
        res = res.text
    ...
    def detailContent(self, ids):
        ...
        videos = []
        result = {}
        res = requests.get(url=xurl + did, headers=headerx,proxies=self.proxy)
        res.encoding = "utf-8"
        res = res.text
        ...
        if source_match:
            ...
            proxied_url = f"{proxy_prefix}{purl}"
            videos.append({
                "vod_id": did,
                "vod_name": '',
                "vod_pic": "",
    ...
    def searchContentPage(self, key, quick, page):
        ...
        res = requests.get(xurl + '/s/page/' + str(page) + '/wd/' + key, headers=header2,proxies=self.proxy)
        res.encoding = "utf-8"
        res = res.text
        videos = []
        doc = BeautifulSoup(res, "html.parser")
