...
class Spider(Spider):
    ...
    def fetch(self, url, timeout=8):
        try:
            # 所有 API 请求均使用代理（如果配置了）
            r = requests.get(
                url,
                headers=self.headers,
                timeout=timeout,
                verify=False,
                proxies=self.proxy if hasattr(self, 'proxy') else {}
            )
            return r.text
        except Exception:
            return ""
    ...
    def clean_item(self, item, source_key, source_name, is_detail=False):
        ...
        # 为封面图片添加代理转发前缀
        if 'vod_pic' in item and item['vod_pic']:
            pic = item['vod_pic'].lstrip('/')
            item['vod_pic'] = f'{self.plp}{pic}'
