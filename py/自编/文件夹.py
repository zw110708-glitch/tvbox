...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        canon = data('link[rel="canonical"]').attr('href')
        play_page = canon or url

        # Dynamic HLS lines: m3u8.push(...) count varies per video
        m3u8_urls = re.findall(r'm3u8\.push\([\"\']([^\"\']+)[\"\']\)', html)
        line_cnt = len(m3u8_urls)
        if line_cnt <= 0:
            line_cnt = 1

        play_from = '$$$'.join([f'线路 {i+1}' for i in range(line_cnt)])
        play_urls = '$$$'.join([f"{title}${play_page}@@{i}" for i in range(line_cnt)])
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        # id can be: "https://.../zh/XXX@@0"  (线路索引)
        idx = 0
        if '@@' in id:
            id, idx_s = id.rsplit('@@', 1)
            try:
                idx = int(idx_s)
            except Exception:
                idx = 0
        ...
        if id.startswith('http'):
            ...
            page_url = f"{self.host}{id}"
        else:
            page_url = f"{self.host}/{id.lstrip('/')}"

        html = self.fetch(page_url)

        # Capture m3u8.push('...') / m3u8.push("...")
        m3u8_urls = re.findall(r'm3u8\.push\([\"\']([^\"\']+)[\"\']\)', html)
        video_url = m3u8_urls[idx] if len(m3u8_urls) > idx else (m3u8_urls[0] if m3u8_urls else '')
