...
class Spider(BaseSpider):
    ...
    def init(self, extend=""):
        ...
        if extend:
            try:
                ...
                self.session.headers.update(self.headers)
                # 获取代理配置，格式要求：{"http": "...", "https": "..."}
                self.proxies = cfg.get("proxies") or {}
            except Exception:
                None
        # 将代理应用到 session（所有通过 session 的请求都会走代理）
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        return {"parse": 0, "playUrl": "", "url": f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{url}', "jx": 0, "header": {"User-Agent": self.headers["User-Agent"], "Referer": self.host + "/home", "Origin": self.host}}
    ...
    def _pic(self, item):
        pic_url = item.get("img_y") or item.get("img_x") or item.get("img") or item.get("cover") or item.get("pic") or ""
        if pic_url:
          pic_url = f"http://127.0.0.1:10079/p/0/127.0.0.1:10172/{pic_url}"
        return pic_url
