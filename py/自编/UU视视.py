...
class Spider(Spider):
    # 导航站入口：utv0.cc ~ utv9.cc 动态轮换，首选 utv0，失效时依次尝试其余九个。
    NAV_HOSTS = ['https://utv%d.cc/' % i for i in range(10)]
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        raw = str(tid or '')
        # 详情页的可点击标签用 search: 前缀作为 type_id，点击后走搜索。
        if raw.startswith('search:'):
            return self.searchContentPage(raw[len('search:'):], None, str(page))
    ...
    def detailContent(self, ids):
        ...
        try:
            ...
            return {'list': [{'vod_id': video_id,
                ...
                'vod_content': self._vod_content(item),
    ...
    def _get_apis(self):
        ...
        for host in self._candidate_hosts():
            try:
                ...
                self.log('AQX host %s unavailable: %s' % (host, error))
        self.api_candidates = self._unique_hosts(discovered + self.api_candidates)
        return list(self.api_candidates)

    def _candidate_hosts(self):
        hosts = []
        if self.web_host:
            hosts.append(self.web_host)
        hosts.extend(self._nav_hosts())
        return self._unique_hosts(hosts)

    def _nav_hosts(self):
        for nav_url in self.NAV_HOSTS:
            try:
                response = self.session.get(
                    nav_url,
                ...
                response.raise_for_status()
                hosts = self._unique_hosts(re.findall(r'data-target="(https?://[^"]+)"', response.text))
                if hosts:
                    self.log('AQX nav %s hosts: %s' % (nav_url, hosts))
                    return hosts
            except Exception as error:
                self.log('AQX nav %s unavailable: %s' % (nav_url, error))
        return []

    def _verified_home(self, host=None):
        host = str(host or self.web_host).strip().rstrip('/')
        response = self.session.get(host + '/home', headers={**self.headers, 'Accept': 'text/html,*/*'},
                                    timeout=18, verify=False)
        response.raise_for_status()
        text = response.text
        # 新版站点把 JS challenge 藏进「正在验证中」中间页，页面 HTML 里不再直接出现
        # /__js_challenge/html，只能靠 decode 后的脚本是否含 postUrl/difficulty 来判定。
        for _ in range(3):
            ...
            verified = self.session.post(urljoin(host + '/', str(task.get('postUrl')).lstrip('/')),
    ...
    def _vod_content(self, item):
        tags = self._clickable_tags(item)
        desc = self._clean_html(item.get('desc') or item.get('description'))
        parts = []
        if tags:
    ...
    def _clickable_tags(self, item):
        tags, seen = [], set()
        for field in ('classify', 'tags'):
            raw = self._text(item.get(field))
            for part in re.split(r'[,，、;；\s]+', raw):
                ...
                tags.append('[a=cr:%s/]%s[/a]' % (
                    json.dumps({'id': 'search:' + name, 'name': name}, ensure_ascii=False), name))
        return tags
