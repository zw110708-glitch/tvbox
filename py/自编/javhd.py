...
class Spider(Spider):
    ...
    def _get(self, url, jsonmode=False, html=False):
        ...
        if not html:
            hdr.update({'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest'})
        try:
        ...
        for _ in range(4):
            try:
                import requests
                r = requests.get(url, headers=hdr, timeout=20, proxies=self.proxy)
                txt = r.text
            except:
                try:
                    ...
                    if self.proxy:
                        ...
                        txt = opener.open(req, timeout=20).read().decode('utf-8', errors='ignore')
                    else:
    ...
    def homeVideoContent(self):
        ...
        for i in j.get('template', []):
            if isinstance(i, dict) and i.get('id'):
                out.append({
                    'vod_id': 'https://javhd.com' + i.get('studioUrl', ''),
    ...
    def categoryContent(self, tid, pg='1', filter=False, extend={}):
        ...
        return {
            'list': [{
                'vod_id': i['link'],
                'vod_name': i['title'],
                'vod_pic': self.plp + i['pic'],
                'vod_remarks': 'VIP' if i['label'] != 'free' else '',
                'vod_content': i['label']
            } for i in items],
            'page': int(pg),
            'pagecount': pc,
            'limit': j.get('per_page', 24),
    ...
    def detailContent(self, ids):
        ...
        return {
            'list': [{
                ...
                'vod_content': desc,
                'vod_play_from': 'javhd',
                'vod_play_url': f'完整版$https://javhd.com/en/player_api?videoId={cid}&is_trailer=0'
            }]
        }
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        return {'parse': 0, 'url': self.plp + s.get('src', '')}
