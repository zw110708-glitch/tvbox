...
class Spider(Spider):
    ...
    def categoryContent(self, cid, page, filter, ext):
        ...
        # 支持详情页可点击标签/人物：cid 可能是 Emby Web 的 list 链接
        # - list/list.html?type=Movie&tagId=52554&serverId=...
        # - list/list.html?type=Movie&personId=122448&serverId=...
        tag_id = None
        person_id = None
        tag_name = None
        ...
        if isinstance(cid, str):
            cid = cid.strip()
            if cid.startswith('list/list.html'):
        ...
        params = {
            ...
            # 普通分类：Movie+Series；标签/人物列表：按Emby Web仅Movie
        ...
        if person_id:
            ...
            # 列表：标签 -> 仅Movie
            params['IncludeItemTypes'] = 'Movie'
            # Emby Items API 没有 tagId 过滤参数（tagId 是Web前端路由用的tag item id）。
            # 做法：先用 tagId 请求其 item 获取标签名，再用 Tags 参数筛选。
            tag_name = None
            tag_item_url = f"{self.baseUrl}/emby/Users/{embyInfos['User']['Id']}/Items/{tag_id}"
            tag_item_params = {
                "X-Emby-Client": self.client,
                "X-Emby-Device-Name": self.device_name,
                "X-Emby-Device-Id": self.device_id,
            ...
            tr = requests.get(tag_item_url, params=tag_item_params, headers=header, timeout=120, proxies={"http": self.proxy, "https": self.proxy})
            if tr.status_code == 200:
                tag_name = self.cleanText((tr.json() or {}).get('Name', ''))
            if tag_name:
        ...
        for video in videoList:
            ...
            videos.append({
                "vod_id": video['Id'],
                "vod_name": name,
                "vod_pic": f"http://127.0.0.1:10079/p/0/127.0.0.1:10174/{self.baseUrl}/emby/Items/{video['Id']}/Images/Primary?maxWidth=400&tag={video['ImageTags']['Primary']}&quality=90" if 'Primary' in video['ImageTags'] else '',
    ...
    def detailContent(self, did):
        ...
        header = self.header.copy()
        header['Content-Type'] = "application/json; charset=UTF-8"
        url = f"http://127.0.0.1:10079/p/0/127.0.0.1:10174/{self.baseUrl}/emby/Users/{embyInfos['User']['Id']}/Items/{did[0]}"
        params = {
            "X-Emby-Client": self.client,
            "X-Emby-Device-Name": self.device_name,
            "X-Emby-Device-Id": self.device_id,
        ...
        vod = {
            ...
            "vod_pic": f'http://127.0.0.1:10079/p/0/127.0.0.1:10174/{self.baseUrl}/emby/Items/{did[0]}/Images/Primary?maxWidth=400&tag={videoInfos["ImageTags"]["Primary"]}&quality=90' if 'Primary' in videoInfos['ImageTags'] else '',
        ...
        # 详情页：标签（可点击，放入vod_content）
        # 按你给的Emby Web格式生成 tagId 列表链接：list/list.html?type=Movie&tagId=xxx&serverId=yyy
        tag_links = []
        ...
        # 注意：这里不再对 Tags(字符串数组)做兜底点击。
        # 因为无法得到 tagId，就无法做到与 Emby Web 一致的精准跳转。
    ...
    def searchContentPage(self, keywords, quick, page):
        ...
        for vod in vodList:
            ...
            videos.append({
                "vod_id": sid,
                "vod_name": name,
                "vod_pic": f'http://127.0.0.1:10079/p/0/127.0.0.1:10174/{pic}',
    ...
    def playerContent(self, flag, pid, vipFlags):
        ...
        if int(self.thread) > 0:
            try:
                self.fetch('http://127.0.0.1:10079/p/0/127.0.0.1:10174/', timeout=120)
            except:
                self.fetch('http:127.0.0.1:10174/go')
            url = f'http://127.0.0.1:10079/p/0/127.0.0.1:10174/{quote(url)}&thread={self.thread}'
    ...
    def _make_list_link(self, *, link_type, value, server_id=''):
        """生成 Emby Web 的列表链接(用于TVBox点击回调后解析)

        示例：list/list.html?type=Movie&tagId=52554&serverId=xxx
             list/list.html?type=Movie&personId=122448&serverId=xxx
        """
