...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        result = {}
        result['class'] = [
            {"type_id": "/", "type_name": "首页"},  # 修改ID
            {"type_id": "/best-videos/", "type_name": "最佳视频"},
            {"type_id": "/top-rated/", "type_name": "最高评分"},
            {"type_id": "/cat/4k-porn/", "type_name": "4K"},
            {"type_id": "/cats/", "type_name": "类别"},  # HTML实际路径
            {"type_id": "/channels/", "type_name": "频道"},  # HTML实际路径
            {"type_id": "/pornstar-list/", "type_name": "明星"},  # HTML实际路径
        ]
        return result
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        if tid == "/":
            # 首页
            videos = self._api_search(query="all", page=int(pg), order="latest", gay="0", per_page=30)
        elif tid.startswith("/cat/"):
            # 分类页面（如：/cat/japanese/）
            category = tid.replace("/cat/", "").strip("/")
            videos = self._api_search(query=category, page=int(pg), order="latest", gay="0", per_page=30)
        elif tid.startswith("/pornstar/"):
            # 明星详情页面（如：/pornstar/angela-white-umh5v/）
            pornstar = tid.replace("/pornstar/", "").strip("/")
            videos = self._parse_pornstar_videos(pornstar, pg)
        elif tid.startswith("/channel/"):
            # 频道详情页面（如：/channel/blacked/）
            channel = tid.replace("/channel/", "").strip("/")
            videos = self._parse_channel_videos(channel, pg)
        elif tid in ["/cats/", "/channels/", "/pornstar-list/"]:
            # 分类列表页面，这些页面不是视频列表，而是分类列表
            # 这里需要返回分类列表而不是视频
            # 暂时返回空视频列表，或者可以解析这些页面获取子分类
            videos = []
        else:
            # 其他特殊分类
    ...
    def detailContent(self, ids):
        ...
        if og_title:
            ...
            title = re.sub(r"\s*(\d+\smin.)$", "", h1).strip() or "爱看AV"
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        r = self.fetch(url, headers=self.headers)
        html_content = r.text
        pattern = r"vid\s*=\s*'([^']+)';\s*[\w*.]+hash\s*=\s*'([^']+)'"
    ...
    def _parse_pornstar_videos(self, pornstar, page):
        # 解析明星页面
        url = f"{self.host}/pornstar/{pornstar}/{page}/"
        r = self.fetch(url, headers=self.headers)
        root = self.html(r.text)
        return self._parse_video_list(root)

    def _parse_channel_videos(self, channel, page):
        # 解析频道页面
        url = f"{self.host}/channel/{channel}/{page}/"
        r = self.fetch(url, headers=self.headers)
        root = self.html(r.text)
        return self._parse_video_list(root)