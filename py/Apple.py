...
class Spider(Spider):
    ...
    def homeContent(self, filter):
        data = self.fetch(f"http://127.0.0.1:10079/p/0/127.0.0.1:10172/{self.host}/api.php/v2.vod/androidtypes", headers=self.headers).json()
        dy = {
            "classes": "类型",
            "areas": "地区",
            "years": "年份",
    ...
    def homeVideoContent(self):
        rsp = self.fetch(f"http://127.0.0.1:10079/p/0/127.0.0.1:10172/{self.host}/api.php/v2.main/androidhome", headers=self.headers).json()
        videos = []
    ...
    def categoryContent(self, tid, pg, filter, extend):
        ...
        rsp = self.fetch(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{self.host}/api.php/v2.vod/androidfilter10086', headers=self.headers, params=params).json()
    ...
    def detailContent(self, ids):
        rsp = self.fetch(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{self.host}/api.php/v3.vod/androiddetail2?vod_id={ids[0]}', headers=self.headers).json()
        v = rsp.get('data', {}) or {}
        urls = v.get('urls') or []
        play_items = []
    ...
    def searchContent(self, key, quick, pg='1'):
        rsp = self.fetch(f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{self.host}/api.php/v2.vod/androidsearch10086?page={pg}&wd={key}', headers=self.headers).json()
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        return {"parse": 0, "url": f'http://127.0.0.1:10079/p/0/127.0.0.1:10172/{id}', "header": header}
