...
class Spider(Spider):
    ...
    def fetch(self, url, headers=None, timeout=15, method='GET', data=None):
        ...
        if method.upper() == 'GET':
            rsp = sess.get(url, headers=req_headers, timeout=timeout, proxies=self.proxies)
        else:
            rsp = sess.post(url, headers=req_headers, timeout=timeout, data=data, proxies=self.proxies)
    ...
    def _extract_resolution(self, item):
        # 精准：仅根据提供的 HTML 选择器提取清晰度
        # <div class="video-text-icon">1080P</div>
        return (item.find('.video-text-icon').text() or '').strip()
    ...
    def parseVideoItem(self, item):
        try:
            ...
            duration = (item.find('.duration').text() or '').strip()
            resolution = self._extract_resolution(item)
            remarks = ''
            if resolution and duration:
                remarks = f"{resolution} | {duration}"
            elif resolution:
                remarks = resolution
            else:
                remarks = duration
            return {
                ...
                "vod_pic": cover if cover and cover.startswith('http') else ("https://theporn.cc" + cover if cover else ''),
                "vod_remarks": remarks
            } if href else None
        except:
            return None
    ...
    def detailContent(self, array):
        ...
        # vod_content 使用可点击标签格式（cr 链接语法）
        tags = []
        data = root('.jav-data-info .categories')
        ...
        for ctg_span in data('span.ctg').items():
            ...
            if a:
                ...
                if name and href:
                    tags.append(f'[a=cr:{json.dumps({"id": href, "name": name})}/]{name}[/a]')
        content_markup = '  '.join(tags) if tags else title
        
        # 详情页 remarks：精准清晰度 + 时长
        duration = (root('.duration').text() or '').strip()
        res_detail = (root('.video-text-icon').text() or '').strip()
        if res_detail and duration:
            remarks = f"{res_detail} | {duration}"
        elif res_detail:
            remarks = res_detail
        else:
            remarks = duration or '未知时长'
        
        return {'list': [{
            'vod_id': tid,
            'vod_name': title,
            'vod_pic': cover,
            'vod_content': f"{content_markup}{title}",
            'vod_actor': '未知演员',
            'vod_director': '未知导演',
            'vod_area': '未知地区', 
            'vod_year': '2024',
